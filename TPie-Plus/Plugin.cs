...
namespace TPie_Plus
{
    public class Plugin : IDalamudPlugin
    {
        ...
        public static ISharedImmediateTexture? RingCenter;
        public static ISharedImmediateTexture? RingArrow;
        ...
        public void LoadPluginTextures()
        {
            try
            {
                ...
                string ringBgPath = Path.Combine(path, "Media", "ring_bg.png");
                Plugin.Logger.Debug($"Plugin Path: {path} | Ring BG Exists: {File.Exists(ringBgPath)}");
                if (File.Exists(ringBgPath))
                {
                    RingBackground = TextureProvider.GetFromFile(ringBgPath);
                }
                string ringCenterPath = Path.Combine(path, "Media", "ring_center.png");
                Plugin.Logger.Debug($"Plugin Path: {path} | Ring Center Exists: {File.Exists(ringCenterPath)}");
                if (File.Exists(ringCenterPath))
                    RingCenter = TextureProvider.GetFromFile(ringCenterPath);
                string ringArrowPath = Path.Combine(path, "Media", "ring_arrow.png");
                Plugin.Logger.Debug($"Plugin Path: {path} | Ring Arrow Exists: {File.Exists(ringArrowPath)}");
                if (File.Exists(ringArrowPath))
                    RingArrow = TextureProvider.GetFromFile(ringArrowPath);
            }
            catch { }
        }
        ...
        {
            _settingsWindow = new SettingsWindow("TPie-Plus Settings");
            ...
            _windowSystem = new WindowSystem("TPie_Plus_Windows");
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            CommandManager.RemoveHandler("/tpiep");
